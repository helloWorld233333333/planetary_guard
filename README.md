...
- Dock 自动收起后，把鼠标移到当前显示器屏幕底部、Dock 大致宽度范围内，连续停留至少 300 ms 即可唤出。托盘“隐藏 Dock”属于手动隐藏，需要通过托盘“显示 Dock”恢复；退出程序后需要重新启动。
- 在多显示器环境中，在另一块屏幕按同样方式触底，Dock 会移动到该屏幕。
- 拖动鼠标或按住鼠标按钮经过底边不会唤出，避免影响拖拽和输入操作。
- 保持“全屏游戏/视频时隐藏”开启：检测到全屏后，该屏幕不会响应触底唤出，退出全屏后恢复。钉钉等保留系统菜单的自绘最大化窗口不再被当作全屏。检测依据是窗口状态与尺寸，不分析屏幕内容；特殊游戏或播放器仍需实机验证。
...
- 自动隐藏、全屏时隐藏、开机启动；
- 显示桌面时强制显示程序坞（默认关闭）：桌面获得焦点后自动显示，并暂停鼠标离开自动隐藏；托盘手动隐藏仍优先；
- 图标大小、悬停放大尺寸、Dock 透明度、减少动画；
- 主题与底部边距。

### 没有打开应用时会显示什么？

Dock 仍保留已固定的应用、快捷方式、文件和文件夹，不要求这些应用正在运行。首次启动默认提供记事本、文件管理器和终端。临时运行应用退出后会消失；若所有固定项目也被移除，则没有图标可显示。

启动时 Dock 会显示，之后是否收起取决于自动隐藏和点击操作。希望回到桌面时始终看到它，可开启“显示桌面时强制显示程序坞”。
